...
const props = defineProps<{ folder: LayoutFolderNode }>()
const emit = defineEmits<{
  (e: 'open', id: string): void
  (e: 'context', event: MouseEvent, folder: LayoutFolderNode): void
  (e: 'drop-layout', folderId: string): void
}>()

const isDragOver = ref(false)
const onDrop = (e: DragEvent) => {
  e.preventDefault(); isDragOver.value = false; emit('drop-layout', props.folder.id)
}
</script>
