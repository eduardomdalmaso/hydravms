...
const availableCameras = ref<CameraPaletteItem[]>([])

onMounted(async () => {
  const cams = await fetchCameras()
  availableCameras.value = cams.map(c => ({
...
<template>
  <div class="vms-flex-col" style="gap: 0.5rem; flex: 1; min-height: 0;">
    ...
    <div v-if="availableCameras.length === 0" class="vms-flex-center" style="padding: 2rem; color: var(--vms-text-dim); font-size: 11px;">
      Nenhuma câmera cadastrada no sistema
    </div>

    <div v-else class="vms-desktop-grid" style="grid-template-columns: repeat(auto-fill, minmax(110px, 1fr)); gap: 0.45rem; overflow-y: auto; padding: 2px;">
      <div
        ...
        :style="{
          ...
          opacity: isUsed(c.id) ? '0.35' : '1', filter: isUsed(c.id) ? 'grayscale(1)' : 'none',
        ...
        :title="isUsed(c.id) ? 'Câmera já no grid' : 'Arraste para o grid'"
