...
defineProps<{ usedCameraIds: string[] }>()
const emit = defineEmits<{ (e: 'addCamera', cam: { id: string; name: string; resolution: string; fps: number }): void }>()

const searchQuery = ref(''), hoveredCam = ref<CamItem | null>(null), hoverX = ref(0), hoverY = ref(0)
const allCameras = ref<CamItem[]>([])

onMounted(async () => {
  const cams = await fetchCameras()
  allCameras.value = cams.map(c => ({
    id: c.id, name: c.name, codec: c.codec || 'H.265', res: c.resolution || '1080P',
    fps: c.fps || 30, ip: c.ip || '127.0.0.1'
  }))
})

const availableCameras = computed(() =>
  allCameras.value.filter(c => !searchQuery.value || c.name.toLowerCase().includes(searchQuery.value.toLowerCase()))
)
...
<template>
  <div class="vms-flex-col" style="gap: 0.5rem; background: #07080c; border: 1px solid var(--vms-border); border-radius: 6px; padding: 0.65rem;">
    ...
    <div v-if="availableCameras.length === 0" class="vms-flex-center" style="padding: 1.5rem; color: var(--vms-text-dim); font-size: 11px;">
      Nenhuma câmera disponível
    </div>

    <div v-else class="vms-desktop-grid" style="grid-template-columns: repeat(auto-fill, minmax(110px, 1fr)); gap: 0.45rem; max-height: 120px; overflow-y: auto; padding: 2px;">
      <div
    ...
    <Teleport to="body">
      <div v-if="hoveredCam" style="position: fixed; z-index: 99999; pointer-events: none; transform: translate(-50%, -105%); width: 220px; background: #0c0f16; border: 1px solid var(--vms-neu-accent-orange); border-radius: 6px; padding: 6px; box-shadow: 0 10px 30px rgba(0,0,0,0.9); display: flex; flex-direction: column; gap: 4px;" :style="{ left: `${hoverX}px`, top: `${hoverY - 12}px` }">
