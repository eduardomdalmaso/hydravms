...
export async function fetchStoragePools(fallback: StoragePoolItem[] = []): Promise<StoragePoolItem[]> {
  try {
    ...
    return data.pools.map((p: any) => ({
      id: p.id,
      name: p.name,
      sourceType: p.source_type || 'LOCAL_DISK',
      role: p.role || 'WARM_ARCHIVE',
      nodeOrServer: p.node_or_server || 'MAQUINA LOCAL',
      pathOrEndpoint: p.path_or_endpoint || '',
      filesystem: p.filesystem || 'XFS',
      totalGb: Math.round(p.total_bytes / (1024 * 1024 * 1024)),
      usedGb: Math.round(p.used_bytes / (1024 * 1024 * 1024)),
  ...
  try {
    const res = await fetch(`${API_BASE}/api/v1/storage/pools`, {
      ...
      body: JSON.stringify({
        ...
        role: payload.role,
        node_or_server: payload.nodeOrServer,
        path_or_endpoint: payload.pathOrEndpoint,
        filesystem: payload.filesystem,
        total_gb: payload.totalGb,
        retention_days: payload.retentionDays || 30
      }),
      signal: AbortSignal.timeout(3000)
    })
    return res.ok
...
export async function deleteRemoteStoragePool(id: string): Promise<boolean> {
  try {
    const res = await fetch(`${API_BASE}/api/v1/storage/pools/${id}`, {
      method: 'DELETE',
      signal: AbortSignal.timeout(3000)
    })
    return res.ok
...
export async function triggerRemoteDrain(): Promise<string> {
  try {
    const res = await fetch(`${API_BASE}/api/v1/storage/spillover/drain`, {
      method: 'POST',
      signal: AbortSignal.timeout(3000)
    })
    ...
    return 'Spillover buffer concluido (modo offline)'
  }
}
