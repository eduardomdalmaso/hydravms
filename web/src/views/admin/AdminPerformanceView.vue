...
const handleAutoDiscover = async () => {
  ...
  showToast(added > 0 ? `[AUTO-DISCOVERY] ${added} nó(s) detectados!` : `[AUTO-DISCOVERY] Nenhum novo nó detectado.`)
}
</script>
  ...
  <div class="vms-content-area vms-flex-col" style="gap: 1rem;">
    ...
    <div class="vms-flex-between" style="align-items: center; gap: 1rem; flex-wrap: wrap;">
      <div class="vms-flex-col" style="gap: 2px;">
        <h2 class="vms-h2" style="color: #ffffff; margin: 0; font-family: var(--vms-font-jetbrains);">{{ t('perf_title') }}</h2>
        <span class="vms-text-mono vms-text-2xs vms-text-dim">{{ t('perf_sub') }}</span>
      </div>
      <div class="vms-flex-row" style="gap: 8px; align-items: center;">
        <button class="vms-btn vms-btn-secondary vms-btn-sm vms-flex-row" :disabled="isAutoScanning" style="font-size: 11px; padding: 4px 10px; align-items: center; gap: 4px;" @click="handleAutoDiscover">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#ff5e3a" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="10.5" cy="10.5" r="6.5"/><line x1="21" y1="21" x2="15.5" y2="15.5"/></svg>
          <span>{{ isAutoScanning ? t('auto_detecting') : t('auto_detect') }}</span>
        </button>
        <button class="vms-btn vms-btn-primary vms-btn-sm" style="font-size: 11px; padding: 4px 10px;" @click="showCreateModal = true">
          {{ t('connect_node') }}
        </button>
        <div class="vms-badge vms-badge-orange" style="font-family: var(--vms-font-jetbrains); font-weight: 700; padding: 4px 10px; font-size: 11px;">{{ t('verification') }}: {{ countdown }}s</div>
        <button class="vms-btn vms-btn-ghost vms-btn-sm" style="padding: 5px 8px; border: 1px solid rgba(255, 94, 58, 0.4); border-radius: 4px; background: rgba(255, 94, 58, 0.08);" :title="t('alerts')" @click="showConfigModal = true">
    ...
    <div v-if="servers.length === 0" class="vms-flex-col" style="align-items: center; justify-content: center; padding: 3rem 1rem; border: 1px dashed var(--vms-border); border-radius: var(--vms-radius-md); background: #0c0e14; gap: 0.75rem;">
      <span class="vms-text-mono vms-text-xs vms-text-dim">{{ t('no_nodes') }}</span>
      <div class="vms-flex-row" style="gap: 0.5rem;">
        <button class="vms-btn vms-btn-secondary vms-btn-sm vms-flex-row" :disabled="isAutoScanning" style="align-items: center; gap: 4px;" @click="handleAutoDiscover">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#ff5e3a" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="10.5" cy="10.5" r="6.5"/><line x1="21" y1="21" x2="15.5" y2="15.5"/></svg>
          <span>{{ isAutoScanning ? t('auto_detecting') : t('auto_detect') }}</span>
        </button>
        <button class="vms-btn vms-btn-primary vms-btn-sm" @click="showCreateModal = true">{{ t('insert_ip_manual') }}</button>
      </div>
    </div>
