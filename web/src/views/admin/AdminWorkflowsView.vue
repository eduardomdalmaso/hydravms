...
<template>
  <div class="vms-flex-col" style="gap: 1rem;">
    <div class="vms-flex-between">
      ...
      <div v-if="!selectedWorkflow" class="vms-flex-row" style="gap: 0.5rem; align-items: center;">
        <input v-model="searchQuery" class="vms-auth-input" style="width: 200px; height: 32px; font-size: 12px; padding: 4px 10px; box-sizing: border-box;" placeholder="Filtrar fluxos..." />
        ...
        <button class="vms-btn vms-btn-secondary" style="height: 32px; padding: 0 10px; box-sizing: border-box;" title="Novo Workflow" @click="isNewWorkflowModalOpen = true"><span>+</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ff5e3a" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></button>
      </div>
    </div>
    ...
    <div class="vms-desktop-container">
      ...
      <div v-else class="vms-desktop-canvas">
        <div v-if="!currentFolder" class="vms-flex-col" style="gap: 0.65rem;">
          ...
          <div v-if="displayedWorkflows.length > 0" class="vms-desktop-grid" style="border-top: 1px solid var(--vms-border); padding-top: 0.65rem;">
