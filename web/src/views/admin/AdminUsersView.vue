  ...
  <div class="vms-flex-col" style="gap: 1rem;">
    ...
    <div class="vms-flex-between">
      <div class="vms-flex-col" style="gap: 2px;">
        ...
        <span v-else class="vms-text-mono vms-text-2xs vms-text-dim">{{ t('users_sub') }}</span>
      </div>
      <div v-if="!selectedUser" class="vms-flex-row" style="gap: 0.5rem; align-items: center;">
        <input v-model="searchQuery" class="vms-auth-input" style="width: 200px; height: 32px; font-size: 12px; padding: 4px 10px; box-sizing: border-box;" :placeholder="t('filter_users')" />
        <button class="vms-btn vms-btn-secondary" style="height: 32px; padding: 0 10px; box-sizing: border-box;" :title="t('new_group')" @click="isFolderModalOpen = true"><span>+</span><svg width="14" height="14" viewBox="0 0 512 512" fill="#ff5e3a"><path d="M64 480H448c35.3 0 64-28.7 64-64V160c0-35.3-28.7-64-64-64H288c-10.1 0-19.6-4.7-25.6-12.8L243.2 57.6C231.1 41.5 212.1 32 192 32H64C28.7 32 0 60.7 0 96V416c0 35.3 28.7 64 64 64z"/></svg></button>
        <button class="vms-btn vms-btn-secondary" style="height: 32px; padding: 0 10px; box-sizing: border-box;" :title="t('new_user')" @click="isWizardOpen = true"><span>+</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ff5e3a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></button>
      </div>
    </div>
